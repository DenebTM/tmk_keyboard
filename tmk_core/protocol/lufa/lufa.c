...
static ringbuf_t sendbuf = {.buffer = sbuf, .head = 0, .tail = 0, .size_mask = SENDBUF_SIZE - 1};
...
void EVENT_USB_Device_ConfigurationChanged(void) {
  ...
  ConfigSuccess &=
      ENDPOINT_CONFIG(CONSOLE_IN_EPNUM, EP_TYPE_INTERRUPT, ENDPOINT_DIR_IN, CONSOLE_EPSIZE, ENDPOINT_BANK_DOUBLE);
#if 0
...
#ifdef VFD_ENABLE
  ConfigSuccess &= ENDPOINT_CONFIG(VFD_OUT_EPNUM, EP_TYPE_BULK, ENDPOINT_DIR_OUT, VFD_EPSIZE, ENDPOINT_BANK_SINGLE);
#endif
}
